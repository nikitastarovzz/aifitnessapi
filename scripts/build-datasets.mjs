...
function buildHealthKitDataset() {
  ...
  const declared = countOf(src, /^\s{4}"case":\s*"/gm);
  ...
  return {
    ...
    declared,
    columns: [
      "identifier", "objcConstant", "group", "abstract", "aggregation", "unitFamily",
      "iosIntroduced", "watchosIntroduced", "deprecated", "appleDocumented", "appleDocs",
    ],
    doc: {
      name: "HealthKit Quantity Type Identifiers 2026",
      version: VERSION,
      publisher: PUBLISHER,
      license: LICENSE,
      methodology:
        `Every HKQuantityTypeIdentifier case, read on ${fetchedOn} from the JSON Apple serves to render developer.apple.com. The identifier, Objective-C constant, group, abstract and platform versions are copied from Apple's payload unchanged. Two columns are DERIVED because Apple states them only in prose and not as machine-readable properties: aggregation (cumulative vs discrete) and unitFamily. Both are matched literally against Apple's own sentences — "measure cumulative values", "use energy units" — and are null where Apple's wording does not state them, never guessed. Aggregation is included because it determines whether HKStatisticsQuery should use .cumulativeSum or a discrete option, and choosing wrong returns a plausible wrong number rather than an error. Apple's abstracts are reproduced for identification of the API surface; the analysis and the aggregation classification are this site's.`,
      ...
      fields: {
        identifier: "Swift case name on HKQuantityTypeIdentifier",
        ...
        aggregation: "DERIVED: cumulative or discrete, per Apple's prose; null where unstated",
        unitFamily: "DERIVED: the unit family Apple names in prose; null where unstated",
...
const built = [buildMatrixDataset(), buildChangesDataset(), buildGlossaryDataset(), buildHealthKitDataset()].map(writeDataset);
